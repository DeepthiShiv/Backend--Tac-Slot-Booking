...
import com.example.demo.model.ReviewerBookingRequest;
import com.example.demo.service.ReviewerBookingService;
...
@RestController
@RequestMapping("/api/reviewer-bookings")
public class ReviewerBookingController {

    @Autowired
    private ReviewerBookingService reviewerBookingService;

    // POST endpoint to book a slot for a reviewer
    @PostMapping
    public ResponseEntity<?> bookSlot(@RequestBody ReviewerBookingRequest bookingRequest) {
        try {
            // Log the incoming request for debugging
            System.out.println("Received reviewer booking request: " + bookingRequest);
            ...
            // Book the slot
            ReviewerBooking booking = reviewerBookingService.bookSlot(bookingRequest.getReviewerId(), date, time);
    ...
    // GET endpoint to fetch all bookings for reviewers
    @GetMapping
    public ResponseEntity<List<ReviewerBooking>> getAllBookings() {
        List<ReviewerBooking> bookings = reviewerBookingService.getAllBookings();
